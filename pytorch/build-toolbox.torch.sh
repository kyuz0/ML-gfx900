...
# Import variables (handles setting ROCM_ARCH to gfx900, etc)
...
TOOLBOX_TAG="docker.io/kyuz0/pytorch-toolbox-gfx900:${TORCH_VERSION}-rocm-${ROCM_VERSION}"
...
podman build -t "${TOOLBOX_TAG}" \
  --build-arg BASE_ROCM_IMAGE="docker.io/kyuz0/rocm-toolbox-gfx900:${ROCM_VERSION}" \
  --build-arg ROCM_ARCH="gfx900" \
