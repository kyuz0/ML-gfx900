...
ARG BASE_TOOLBOX="docker.io/kyuz0/rocm-toolbox-gfx900:${ROCM_VERSION}"

# The source builder uses the exact same base toolbox
FROM ${BASE_TOOLBOX} AS builder
ARG ROCM_DOCKER_ARCH="gfx900"
...
# Compile Llama natively for gfx900 using the ROCm tools mapped into the host
